...
export async function runSetup() {
    ...
    try {
        ...
        if (!channel.ok) {
            ...
            console.log('Fix that, then run `npx agent-wire setup` again — it resumes here.');
            return 1;
        }
        ...
        console.log('\nAdd this to your MCP client (Claude Code: `claude mcp add agent-wire -- npx -y agent-wire serve`):');
...
export async function runDoctor() {
    ...
    if (!config?.bot_token) {
        console.log('not configured — run `npx agent-wire setup`');
        return 1;
    }
