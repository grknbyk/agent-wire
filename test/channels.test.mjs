...
test('only a channel that was told to be on is on', () => {
    const names = activeChannels({ channels: [{ name: 'a' }, { name: 'b', active: true }, { name: 'c', active: false }] })
        .map((channel) => channel.name);

    assert.deepEqual(names, ['b'], 'a said nothing, so a says nothing');
});

test('the default view shows only the channels somebody opened', () => {
    ...
    assert.deepEqual(seen.sort(), ['agent-crm']);
});
...
// A client keeps one session id across a compact and a --resume, so the entry
// written under it is still there afterwards. What must NOT happen is the choice
// leaking sideways: a second window is a new session and starts silent.
test('a mode set in a session stays put and does not leak to the next one', () => {
    setChannelMode('agent-wms', 'read');

    const config = loadConfig();
    const wms = config.channels[0];

    assert.equal(channelMode(config, wms, scopeId()), 'read', 'the session keeps what it set');
    assert.equal(channelMode(config, wms, 'a-window-opened-later'), 'off', 'a new session starts silent');
});
...
test('a channel nobody has opened in this session is silent', () => {
    const channel = { name: 'agent-wms' };
    assert.equal(channelMode({ channels: [channel], scopes: {} }, channel, 'session-a'), 'off');
});

test('a channel written before modes existed keeps the on it was given', () => {
    const channel = { name: 'agent-wms', active: true };
