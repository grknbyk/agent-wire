...
test('the three modes are offered as prompts, not as tools', async () => {
    ...
    assert.deepEqual(listed.result.prompts.map((prompt) => prompt.name), ['off', 'ask', 'read', 'on', 'status']);
...
// The shell has taken `on` since before there were three modes. A word that works
// in one place and not the other is the surprise.
test('on is offered as a prompt and means ask', async () => {
    const [answer] = await serve([
        { jsonrpc: '2.0', id: 1, method: 'prompts/get', params: { name: 'on', arguments: { channel: 'wms-agents' } } },
    ]);

    assert.match(answer.result.messages[0].content.text, /`agent-wire ask wms-agents`/);
});
